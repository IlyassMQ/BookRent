...
@section('title', '#' . $tag->name . ' - Related Books')
...
<div class="max-w-7xl mx-auto px-4 py-8">
    ...
    <div class="relative overflow-hidden bg-gradient-to-r from-amber-50 to-amber-100/30 rounded-2xl shadow-lg border border-amber-200 p-8 mb-8">
        ...
        {{-- Decorative tag icon background --}}
        <div class="absolute top-0 right-0 opacity-10 transform -translate-y-8 translate-x-8">
            <svg class="w-48 h-48 text-amber-800" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1" d="M7 7h.01M7 3h5c.512 0 1.024.195 1.414.586l5 5a2 2 0 010 2.828l-7 7a2 2 0 01-2.828 0l-5-5A1.994 1.994 0 013 12V7a4 4 0 014-4z"></path>
            </svg>
        </div>
            ...
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <div class="flex items-center gap-2 mb-2">
                        ...
                        <p class="text-sm font-semibold text-amber-700 uppercase tracking-wider">Book Tag</p>
                    </div>
                    <div class="flex items-center gap-3 mb-2">
                        <div class="px-4 py-2 bg-white/80 backdrop-blur-sm rounded-xl border border-amber-200 shadow-sm">
                            <span class="text-2xl md:text-3xl font-bold serif-font text-[#2C1810]">#{{ $tag->name }}</span>
                        </div>
                    </div>
                    <p class="text-stone-600">
                        <span class="inline-flex items-center gap-2">
                            ...
                            {{ $books->total() }} {{ Str::plural('book', $books->total()) }} tagged with "{{ $tag->name }}"
                        </span>
                    </p>
                </div>
                ...
                <div class="flex gap-3">
                    <div class="bg-white/80 backdrop-blur-sm rounded-xl px-4 py-2 border border-amber-200">
                        <p class="text-xs text-amber-700 font-semibold">Tag Popularity</p>
                        <p class="text-2xl font-bold text-amber-800">{{ $tag->books_count ?? $books->total() }}</p>
                    </div>
                </div>
            </div>
        </div>
               ...
               class="group bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-amber-100 hover:border-amber-300 transform hover:-translate-y-1">
                ...
                <div class="relative h-64 bg-gradient-to-br from-amber-50 to-stone-100 overflow-hidden">
                    ...
                    <div class="absolute top-3 left-3">
                        <span class="inline-flex items-center gap-1 px-2 py-1 text-xs font-semibold bg-amber-600 text-white rounded-lg shadow-md">
            ...
            <div class="col-span-full">
                <div class="bg-white rounded-2xl shadow-xl border border-amber-100 p-12 text-center">
                    <div class="flex flex-col items-center gap-3">
                        ...
                        <p class="text-stone-400">We don't have any books tagged with "{{ $tag->name }}" yet.</p>
    ...
    @if($books->isNotEmpty())
        <div class="mt-12 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div class="bg-gradient-to-r from-amber-50 to-transparent rounded-xl p-5 border border-amber-100">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-sm font-semibold text-[#2C1810]">Popular Tags</p>
                        <div class="flex flex-wrap gap-2 mt-2">
                            @php
                                $allTags = $books->flatMap(function($book) {
                                    return $book->tags;
                                })->unique();
                            @endphp
                            @foreach($allTags->take(10) as $relatedTag)
                                @if($relatedTag->id != $tag->id)
                                    <a href="{{ route('books.tag', $relatedTag->id) }}" 
            ...
            {{-- Tag Information --}}
            <div class="bg-gradient-to-r from-blue-50 to-transparent rounded-xl p-5 border border-blue-100">
                <div class="flex items-start gap-3">
                    ...
                    <div>
                        <p class="text-sm font-semibold text-[#2C1810]">About #{{ $tag->name }}</p>
                        <p class="text-xs text-stone-600 mt-1">
                            Books tagged with "{{ $tag->name }}" cover various topics related to this theme. 
                            Explore our collection of {{ $books->total() }} {{ Str::plural('book', $books->total()) }} 
                            featuring this tag, available for purchase or rental.
                        </p>
                    </div>
                </div>
            </div>
